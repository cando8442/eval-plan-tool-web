...
PHRASE_FIXES = {
    "수송 과 정의 종 류 와 특 징을": "수송 과정의 종류와 특징을",
    "산 업 화, 도 시 화 가 생 활 공 간 과 생 활 양 식의 변 화 와 관 련 이 있 음을": "산업화, 도시화가 생활공간과 생활양식의 변화와 관련이 있음을",
    "공 간의 변 화에 따른 사회 변 화의 양 상 과 문 제 점을": "공간의 변화에 따른 사회 변화의 양상과 문제점을",
    "이 해 하 고 사 례를": "이해하고 사례를",
    "양 상에 대 한 조 사 결 과를": "양상에 대한 조사 결과를",
    "구성하는원소들이": "구성하는 원소들이",
    "일상생활에서활용되는": "일상생활에서 활용되는",
    "일상생활의문제해결에": "일상생활의 문제해결에",
    "특 징 및 생 태 계 에 서 일 어 나 는 물 질 의 순 환 과 에 너 지 의 흐 름 을 비 교 하 여 설 명 할": "특징 및 생태계에서 일어나는 물질의 순환과 에너지의 흐름을 비교하여 설명할",
...
def parse_standards(lines):
    ...
    for line in lines:
        ...
        if m_area and not CODE_RE.search(line):
            flush()
            # 영역 헤더가 성취기준 한복판(쪽 경계)에서 끼어드는 판이 있다.
            # 진행 중인 성취기준을 끊지 않는다. 영역은 어차피 코드로 다시 나눈다.
            cur_area = {"unit": despace(m_area.group(2)), "standards": []}
            areas.append(cur_area)
            continue
        m_code = CODE_RE.search(line)
        ...
        if INQUIRY_HEAD_RE.match(line):
            ...
            if tail:
                bucket = [tail]
            continue
        # '...관계를 기준 A 개인과 공동체의...'처럼 본문과 첫 등급이 한 줄에
        # 붙어 나오는 판. 본문을 아직 모으는 중이고 A가 없을 때만 쪼갠다.
        if cur_std is not None and mode == 'text' and 'A' not in cur_std['levels']:
            m_in = re.match(r'^(.+?)\s+A\s+(\S.{20,})$', line.strip())
            if m_in:
                bucket.append(m_in.group(1))
                flush()
                mode, bucket = 'A', [m_in.group(2)]
                continue
        m_lv = LEVEL_RE.match(line)
        if m_lv and cur_std is not None:
            flush()
...
def parse_area_levels(lines, area_names, boundary='header'):
    ...
    plain_names = {squish(n) for n in area_names}
    trait_plain = {squish(k): v for k, v in TRAIT_MAP.items()}
    # 영역 헤더 '(n) 영역명'을 먼저 전부 모아 둔다. 표 첫 칸에도 같은 이름이
    # (줄바꿈으로 쪼개진 채) 되풀이되는데, 그게 가장 확실한 영역 전환 신호다.
    all_headers = []
    for ln in lines:
        ma = AREA_RE.match(ln)
        if ma:
            nm = despace(ma.group(2))
            if nm and squish(nm) not in {squish(x) for x in all_headers}:
                all_headers.append(nm)
    header_squish = {squish(x): x for x in all_headers}
    ...
    while idx + 1 < len(lines):
        idx += 1
        line = lines[idx]
        # 표 첫 칸의 영역명(최대 세 줄로 쪼개짐)과 정확히 맞으면 그 영역으로 넘어간다.
        hit_n = 0
        for span in (3, 2, 1):
            joined = squish(''.join(lines[idx:idx + span]))
            if joined in header_squish:
                hit_n = span
                break
        if hit_n:
            flush()
            cur_unit = header_squish[squish(''.join(lines[idx:idx + hit_n]))]
            cur_grade = cur_trait = None
            pending_name = None
            idx += hit_n - 1
            continue
        ...
        if m_split:
            head, letter = m_split.group(1), m_split.group(2)
            # '문화와 다양성 A'처럼 영역명과 등급이 한 줄에 붙어 나오는 판.
            if head in header_squish:
                flush()
                cur_unit = header_squish[head]
                cur_grade, cur_trait = letter, None
                pending_name = None
                continue
            # 앞부분이 현재 영역명이나 아직 전환하지 않은 영역명의 조각일 때도 등급으로 본다.
            near = [x for x in (cur_unit, pending_name) if x]
            if head and any(head in squish(x) for x in near):
                flush()
                cur_grade, cur_trait = start_area_if_A(letter), None
...
DROP_RE = [re.compile(p) for p in DROP_PATTERNS]
AREA_RE = re.compile(r'^\((\d)\)\s*(.+)$')
CODE_RE = re.compile(r'\[((?:10|12)[\uac00-\ud7a3\u2160-\u217f]+\d?(?:-\d{2})?\d{0,2}-\d{2})\]')
LEVEL_RE = re.compile(r'^([A-E])(?:\s+(.*))?$')
# 등급 문자 뒤에 공백 없이 본문이 붙는 판(예: "DD NA의 구조와…")을 위한 보조 패턴
LEVEL_TIGHT_RE = re.compile(r'^([A-E])(\S.*)$')
INQUIRY_HEAD_RE = re.compile(r'^<\s*탐\s*구\s*활\s*동\s*>')
...
REPAIRS = {
    ...
    "고감상할수": "고 감상할 수",
    "단어및대부분단": "단어 및 대부분 단",
    "설명할줄알고,유사한변": "설명할 줄 알고, 유사한 변",
    "여발표하고질": "여 발표하고 질",
    "찾아적절하게수정할수": "찾아 적절하게 수정할 수",
    "찾고그이": "찾고 그 이",
    "찾아그이": "찾아 그 이",
    "비교적명확하게전달할수": "비교적 명확하게 전달할 수",
    "다양한관점": "다양한 관점",
    "분석할때화자나필": "분석할 때 화자나 필",
    "포용하려고노력": "포용하려고 노력",
    "나타난일이나": "나타난 일이나",
    "여비교적원활하게협업할수": "여 비교적 원활하게 협업할 수",
    "음함수와간단한역함": "음함수와 간단한 역함",
    "다양한장르와매체속문화현상이나문화적산": "다양한 장르와 매체 속 문화 현상이나 문화적 산",
    "활용하여비교적명확하게묘사할수": "활용하여 비교적 명확하게 묘사할 수",
    "반성할수": "반성할 수",
    "맞게정확하게": "맞게 정확하게",
    "짧고간결한이야기나서사및운": "짧고 간결한 이야기나 서사 및 운",
    "활용하여비교적정확하게재구성하여요약할수": "활용하여 비교적 정확하게 재구성하여 요약할 수",
    "활용하여비교적정확하게파악할수": "활용하여 비교적 정확하게 파악할 수",
    "점검할수있으며개선할": "점검할 수 있으며 개선할",
    "듣고,사실정": "듣고, 사실 정",
    "듣고,사실정보와맥락정": "듣고, 사실 정보와 맥락 정",
    "양상과문제": "양상과 문제",
    "인식할수": "인식할 수",
    "설계할수": "설계할 수",
    "판단할수": "판단할 수",
...
TRAILING_GRADE_RE = re.compile(r'^(.*[.가-힣)\]])[ ]*([A-E])$')


def split_trailing_grades(lines):
    """줄 끝에 홀로 붙은 등급 문자를 다음 줄로 떼어 낸다.

    '...그릴 수 있다. A', '...적극적으로 보인다.B'처럼 다음 등급 표시가 앞 줄
    끝에 붙어 나오는 판이 흔하다. 줄 단위로 읽으면 그 등급이 통째로 묻힌다.
    """
    out = []
    for ln in lines:
        m = TRAILING_GRADE_RE.match(ln.rstrip())
...
def parse_standards(lines):
    """성취기준별 성취수준 -> [{unit, standards:[{code,text,inquiry,levels}]}]"""
    lines = split_trailing_grades(lines)
    areas = []
    cur_area = None
    cur_std = None
    bucket = []
    ...
    def flush():
        ...
        if mode is not None and bucket and (cur_std is not None or mode == 'area_inquiry'):
            ...
            if mode == 'text':
                ...
                # 쪽 안에서 단 순서가 뒤바뀌어, 앞 성취기준의 남은 등급이 다음
                # 성취기준 뒤에 붙어 나오는 판이 있다(공통수학1 03-03의 C·D·E).
                # 이미 채워진 등급에 또 쓰려 하면 그 등급이 비어 있는 직전
                # 성취기준을 찾아 되돌린다.
                if cur_std['levels'].get(mode):
                    owner = None
                    for a2 in areas:
                        for st2 in a2['standards']:
                            if st2 is cur_std:
                            # 원문이 비어 있는 칸('')은 후보가 아니다 - 그 등급을
                            # 아예 읽지 못한 성취기준만 되돌릴 자리다.
                                break
                            if mode not in st2['levels']:
                                owner = st2
                        else:
                            continue
                        break
                    if owner is not None:
                        owner['levels'][mode] = val
                        bucket = []
                        return
                cur_std['levels'][mode] = val
        bucket = []
    ...
    for line in lines:
        ...
        if m_code and line.lstrip().startswith('['):
            ...
            if cur_area is None:
                ...
                areas.append(cur_area)
            cur_area["standards"].append(cur_std)
            # '[12기하03-05] 좌표공간 A 좌표공간에서 벡터를...'처럼 코드 줄에
            # 본문과 첫 등급이 함께 오는 판이 있다.
            m_first = re.match(r'^(.+?)\s+A\s+(\S.{20,})$', line.strip())
            if m_first:
                cur_std['text'] = despace(m_first.group(1))
                cur_std['levels']['A'] = ''
                mode, bucket = 'A', [m_first.group(2)]
            else:
                mode, bucket = 'text', [line]
            continue
        if INQUIRY_HEAD_RE.match(line):
        ...
        if m_lv and cur_std is not None:
            flush()
            mode = m_lv.group(1)
            # 원문에서 그 등급 칸이 비어 있는 판이 흔하다(수학·영어에 특히 많다).
            # 등급 문자를 만난 순간 칸을 열어 둬야 '원문 공란'과 '못 읽음'이 구분된다.
            cur_std['levels'].setdefault(mode, '')
            bucket = [m_lv.group(2)] if m_lv.group(2) else []
            continue
        # '...설명할 수 있다.D'처럼 다음 등급 문자가 앞 문장 끝에 붙어 나오는 판.
        # (원문에서 그 등급 칸이 비어 있을 때 이렇게 붙는다.)
        if cur_std is not None and mode in GRADES:
            ...
            if tail_m and next_grade(mode) == tail_m.group(2):
                ...
                flush()
                mode, bucket = tail_m.group(2), []
                cur_std['levels'].setdefault(tail_m.group(2), '')
                continue
        if cur_std is not None:
            nxt = next_grade(mode)
            m_t = LEVEL_TIGHT_RE.match(line)
...
AREA_NO_RE = re.compile(r'^(?:10|12)[\uac00-\ud7a3\u2160-\u217f]+\d?-?(\d{2})-\d{2}$')
...
def main():
    ...
    areas = parse_standards(page_lines(PDF, std_lo, std_hi))
    area_names = [a["unit"] for a in areas]
    # 영역별 성취수준 절이 아예 없는 과목(국어과 선택과목 등)은 0 0으로 부른다.
    lvl_lines = page_lines(PDF, area_lo, area_hi) if area_lo and area_hi else []
    ...
    if not lvl_lines:
        ...
        if pick != '헤더 보류':
            ...
            print('경고: 영역 수가 성취기준 코드와 다름 (%d != %d)'
                  % (len(area_levels), expected))
...
DROP_RE = [re.compile(p) for p in DROP_PATTERNS]
AREA_RE = re.compile(r'^\((\d)\)\s*(.+)$')
CODE_RE = re.compile(r'\[((?:10|12)[\uac00-\ud7a3\u2160-\u217fIVX]+\d?(?:-\d{2})?\d{0,2}-\d{2})\]')
LEVEL_RE = re.compile(r'^([A-E])(?:\s+(.*))?$')
# 등급 문자 뒤에 공백 없이 본문이 붙는 판(예: "DD NA의 구조와…")을 위한 보조 패턴
LEVEL_TIGHT_RE = re.compile(r'^([A-E])(\S.*)$')
INQUIRY_HEAD_RE = re.compile(r'^<\s*탐\s*구\s*활\s*동\s*>')
...
ROMAN_LATIN = [('VIII', 'Ⅷ'), ('VII', 'Ⅶ'), ('III', 'Ⅲ'), ('VI', 'Ⅵ'),
               ('IV', 'Ⅳ'), ('IX', 'Ⅸ'), ('II', 'Ⅱ'), ('V', 'Ⅴ'), ('X', 'Ⅹ'), ('I', 'Ⅰ')]


def normalize_roman(code):
    """코드 안의 로마숫자를 라틴 대문자가 아닌 로마숫자 문자로 통일한다."""
    head, sep, tail = code.partition('-')
    for latin, roman in ROMAN_LATIN:
        if latin in head:
            head = head.replace(latin, roman)
            break
...
def parse_standards(lines):
    ...
    for line in lines:
        ...
        if m_code and line.lstrip().startswith('['):
            flush()
            # 같은 과목 안에서도 로마숫자를 라틴 대문자로 조판한 판이 섞여 있다
            # ([12영II-01-01] vs [12영Ⅱ-01-02]). 코드 표기를 하나로 맞춘다.
            code = normalize_roman(m_code.group(1))
            cur_std = {"code": code, "text": "", "inquiry": [],
                       "levels": collections.OrderedDict()}
            if cur_area is None:
                cur_area = {"unit": "(영역 미상)", "standards": []}
                areas.append(cur_area)
...
AREA_NO_RE = re.compile(r'^(?:10|12)[\uac00-\ud7a3\u2160-\u217fIVX]+\d?-?(\d{2})-\d{2}$')
