...
def strip_table_head(value):
    """셀 끝에 딸려 온 표 머리(다음 영역명)를 떼어 낸다.

    성취수준 본문은 언제나 '~다.'로 끝난다. 마지막 줄이 짧고 마침표로 끝나지
    않으면 내용이 아니라 표 머리이므로 버린다."""
    parts = (value or '').split(chr(10))
    while len(parts) > 1:
        tail = parts[-1].strip()
        if 0 < len(tail) <= 24 and not tail.endswith(('.', '다', '음', '함', '것')):
            parts.pop()
        else:
            break
...
def main():
        ...
        collections.OrderedDict([
            ...
            ("levels", collections.OrderedDict(
                (g, collections.OrderedDict(
                    (t, strip_table_head((area_lv.get(a["unit"], {}).get(g) or {}).get(t, "")))
                    for t in TRAITS))
                for g in GRADES)),
        ]) for i, a in enumerate(areas)
    ]
    ...
    d["achievement_levels"] = collections.OrderedDict(
        (g, collections.OrderedDict(
            (t, "\n".join(
                "[%s] %s" % (labels[i], strip_table_head((area_lv.get(a["unit"], {}).get(g) or {}).get(t, "")))
                for i, a in enumerate(areas)
    ...
    for i, a in enumerate(areas):
        ...
        d["minimum_achievement"].append(collections.OrderedDict([
            ...
            ("영역별성취수준E", " ".join(
                strip_table_head(e[t]).replace(chr(10), " ") for t in TRAITS if e.get(t))),
            ("성취기준별성취수준E", [
                "%s-%s %s" % (split_code(s["code"])[2], split_code(s["code"])[3],
                              s["levels"].get("E", "")) for s in a["standards"]]),
            ("최소능력수행특성", collections.OrderedDict(
                (t, strip_table_head(e.get(t, ""))) for t in TRAITS)),
        ]))
    ...
    months = sorted((d.get('units_by_month') or {}), key=lambda x: int(x))
    # 단원명 표기가 원문과 달라 못 붙은 행은 순서로 맞춘다. 월 수와 영역 수가
    # 달라도(영역이 더 많은 과목이 흔하다) 앞에서부터 채워 옛 성취기준을 남기지 않는다.
    if matched < len(months):
        ...
        matched = min(len(months), len(labels))
...
def main():
    ...
    # 코드 체계가 둘이다: 진로선택 '12유전01-01'과 공통 '10통과1-01-01'.
    # 자릿수로 자르면 안 되고 정규식으로 쪼갠다. sep은 영역 번호 앞 하이픈 유무.
    CODE_RE = re.compile(r'^((?:10|12)[\uac00-\ud7a3\u2160-\u217f]+(?:\d(?=-))?)(-?)(\d{2})-(\d{2})$')
    def split_code(code):
        m = CODE_RE.match(code)
        if m:
            return m.group(1), m.group(2), m.group(3), m.group(4)
    ...
    areas = P["areas"]
    # 성취기준별 표에 영역 헤더가 없는 과목(국어과 선택과목 등)은 영역이 하나뿐이다.
    for a in areas:
        if a["unit"].startswith("(영역"):
            a["unit"] = subject
    labels = [unit_label(i, a["unit"]) for i, a in enumerate(areas)]
    area_lv = {a["unit"]: a["levels"] for a in P["area_levels"]}
    ...
    has_area = bool(P.get("area_levels"))

    # 영역별 성취수준 (문서에 그 절이 없는 과목은 비운다)
    d["area_achievement_levels"] = [
        collections.OrderedDict([
            ("unit", labels[i]),
            ("levels", collections.OrderedDict(
    ...
    # 학기 단위 성취수준. 영역별 성취수준이 있으면 그 원문을 범주별로 이어붙이고,
    # 없는 과목은 성취기준별 성취수준을 등급별로 모아 한 칸에 담는다(범주 구분이
    # 원문에 없으므로 표를 3행으로 쪼갤 근거가 없다).
    if has_area:
        d["achievement_levels"] = collections.OrderedDict(
            (g, collections.OrderedDict(
                (t, chr(10).join(
                    "[%s] %s" % (labels[i],
                                 strip_table_head((area_lv.get(a["unit"], {}).get(g) or {}).get(t, "")))
                    for i, a in enumerate(areas)
                    if (area_lv.get(a["unit"], {}).get(g) or {}).get(t)))
                for t in TRAITS))
            for g in GRADES)
        ...
        d["achievement_levels"] = collections.OrderedDict(
            (g, chr(10).join(
                "[%s-%s] %s" % (split_code(s["code"])[2], split_code(s["code"])[3],
                                s["levels"].get(g, ""))
                for a in areas for s in a["standards"] if s["levels"].get(g)))
            for g in GRADES)
    ...
    for i, a in enumerate(areas):
        ...
        d["minimum_achievement"].append(collections.OrderedDict([
            ("영역", "[%s%s%s] %s" % (code_prefix, code_sep, area_no, a["unit"])),
            # 영역별 성취수준이 없는 과목은 그 영역 성취기준들의 E수준을 이어붙인다.
            ("영역별성취수준E",
             " ".join(strip_table_head(e[t]).replace(chr(10), " ")
                      for t in TRAITS if e.get(t)) if e else
             " ".join(s["levels"].get("E", "") for s in a["standards"] if s["levels"].get("E"))),
            ("성취기준별성취수준E", [
                "%s-%s %s" % (split_code(s["code"])[2], split_code(s["code"])[3],
                              s["levels"].get("E", "")) for s in a["standards"]]),
            # 범주별 최소 능력 수행 특성은 영역별 성취수준에만 있다.
            ("최소능력수행특성", collections.OrderedDict(
                (t, strip_table_head(e.get(t, ""))) for t in TRAITS) if e else {}),
        ]))
    d["minimum_achievement_overall"] = " ".join(
        (area_lv.get(a["unit"], {}).get("E") or {}).get("지식이해", "").replace("\n", " ")
        for a in areas).strip()
    ...
    # 단원명 표기가 원문과 달라 못 붙은 행은 순서로 맞춘다(월 수와 영역 수가 같을 때만).
    months = sorted((d.get('units_by_month') or {}), key=lambda x: int(x))
    # 단원명 표기가 원문과 다르면 순서로 맞춘다. 영역이 월보다 적은 과목(국어과
    # 선택과목은 영역이 하나뿐이다)은 같은 단원이 되풀이되지 않도록 성취기준을
    # 월 수만큼 고르게 나눠 담는다. 어차피 교사가 진도에 맞춰 고쳐 쓰는 칸이다.
    if matched < len(months):
        if len(labels) >= len(months):
            for idx, m in enumerate(months):
                info = d['units_by_month'][m]
                info['unit'] = labels[idx]
                info['standards'] = list(by_label[labels[idx]])
        else:
            flat = [(labels[i], s['text'])
                    for i, a in enumerate(areas) for s in a['standards']]
            n = len(months)
            size, rest = divmod(len(flat), n)
            pos = 0
            for idx, m in enumerate(months):
                take = size + (1 if idx < rest else 0)
                chunk = flat[pos:pos + take]
                pos += take
                info = d['units_by_month'][m]
                info['unit'] = chunk[0][0] if chunk else labels[-1]
                info['standards'] = [t for _, t in chunk]
        matched = len(months)
